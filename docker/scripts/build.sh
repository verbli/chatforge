...
BUILD_NUMBER_FILE="${BASE_PATH}/build_number.txt"
...
if [[ "$TARGETS" == *"linux"* || "$TARGETS" == "all" ]]; then
    ...
    /build-scripts/build_linux.sh "$VERSION" "$BUILD"
fi
...
if [[ "$TARGETS" == *"android-apk"* || "$TARGETS" == "all" ]]; then
    ...
    /build-scripts/build_apk.sh "$VERSION" "$BUILD"
fi
...
if [[ "$TARGETS" == *"android-bundle"* || "$TARGETS" == "all" ]]; then
    ...
    /build-scripts/build_bundle.sh "$VERSION" "$BUILD"
fi

if [[ "$TARGETS" == *"web"* || "$TARGETS" == "all" ]]; then
    echo "Building Web release..."
    /build-scripts/build_web.sh "$VERSION" "$BUILD"
fi
